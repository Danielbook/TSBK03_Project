...
void main() {
    ...
    vec3 atmosphereColor = vec3(0, 11/255, 255/255);
    ...
    vec3 diffuse  = kd * atmosphereColor * max(0.0, dot(vNormal, lightVector));
    vec3 specular = ks*(pow(RdotV, shinyness))*lightColor;

    vec3 finalColor = ambient+specular;

    float atmosphereNoise = pnoise(0.02*vPosition + 0.01*time, vec3(25.0));
    float atmosphereNoise2 = 0.5+0.5*pnoise(0.1*vPosition + vec3(0.4, 0.26, 0.66)*time, vec3(10.0));
    ...
    outColor = vec4(finalColor, atmosphereOpacity*atmosphereNoise);
